...
pin_LED = 13 
pin_Servo = 9
CONFIG_CS = np.array([30,1,0,0]) # configuration for conditioned stimulus 
CONFIG_US = np.array([30,1,1,0]) # configuration for unconditioned stimulus (feeding)
...
class Assay():
    ...
    def __init__(self, fname, nrepeat = 10, ard_port = '/dev/ttyACM1', pump_port = '/dev/ttyUSB0'):
        self.__observers = [] # define an observer
        self.step = -1 # before execution
        self.conf_label = 'R'+ 'CUI'*nrepeat
        print self.conf_label
        self.n_session = 1+ nrepeat*3
    ...
    def __construct__(self):
        ...
        self.CS_config[inter_pos,0] = np.random.randint(10, 60, size = len(inter_dur)) 
...
#         self.terminate_step()
    ...
    def run_through(self):
        ...
        for ii in range(self.n_session):
            self.run_step()
            time.sleep(self.CS_config[ii,0])
            ii +=1
